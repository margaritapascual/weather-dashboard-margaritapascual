...
from features.theme_switcher          import create_theme_menu, THEMES
...
class WeatherGUI:
    ...
    def _build_ui(self):
        # Menu bar with Theme submenu
        create_theme_menu(self.root, self._apply_theme)
        ...
        self.current_frame = tk.Frame(self.container, width=120)
        ...
        # Buttons: trends, history, temp charts
        ...
        for label, cmd in [
            ...
            ("7-Day Temp",  lambda: show_temp_chart(
                                self.chart_frame,
                                self.storage.get_history(30),
                                7)),
            ("30-Day Temp", lambda: show_temp_chart(
                                self.chart_frame,
                                self.storage.get_history(30),
                                30)),
        ]:
    ...
    def _apply_theme(self, theme_name):
        """Recolor every widget according to THEMES[theme_name]."""
        ...
        for w in self.ctrl.winfo_children():
            if isinstance(w, (tk.Label, tk.Button, tk.Entry)):
        ...
        # Also update any Treeview (history) colors
    ...
    def _reload(self):
        ...
        for f in (self.current_frame, self.forecast_frame, self.chart_frame):
    ...
    def _load_and_display(self):
        ...
        try:
            ...
            tk.Label(self.current_frame,
                     text=f"Feels like {round(current['feels_like'])}°F").pack()
            tk.Label(self.current_frame,
                     text=current["weather"][0]["description"].capitalize()).pack(pady=(5,0))
            tk.Label(self.current_frame,
                     text=f"Humidity: {current['humidity']}%").pack()
            tk.Label(self.current_frame,
                     text=f"UV Index: {current['uvi']}").pack(pady=(0,5))

            # 7-day forecast & default trend
    ...
    def _plot_trend(self, period: str):
        ...
        ax1.set_ylabel("Precip (mm)"); ax2.set_ylabel("Humidity (%)")
    ...
    def _aggregate_weekly(self, dates, precs, hums):
        ...
        for i in range(0, len(dates), 7):
            chunk_p = precs[i:i+7]; chunk_h = hums[i:i+7]
            label   = f"W{(i//7)+1}"
